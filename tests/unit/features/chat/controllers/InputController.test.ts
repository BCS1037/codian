...
function createMockFileContextManager() {
  return {
    ...
    consumeDroppedReferences: jest.fn(),
...
describe('InputController - Message Queue', () => {
  ...
  describe('Queuing messages while streaming', () => {
    ...
    it('should queue message when isStreaming is true', async () => {
      const fileContextManager = deps.getFileContextManager()! as any;
      ...
      expect(deps.onDraftConsumed).toHaveBeenCalled();
      expect(fileContextManager.consumeDroppedReferences).toHaveBeenCalled();
    });
