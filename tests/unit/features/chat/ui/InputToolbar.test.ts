...
describe('ModelSelector', () => {
  ...
  it('should keep provider groups ordered while reversing models within each group', () => {
    callbacks.getUIConfig.mockReturnValue({
      ...createMockUIConfig(),
      getModelOptions: jest.fn().mockReturnValue([
        { value: 'codex-old', label: 'Codex Old', group: 'Codex' },
        { value: 'codex-new', label: 'Codex New', group: 'Codex' },
        { value: 'claude-old', label: 'Claude Old', group: 'Claude' },
        { value: 'claude-new', label: 'Claude New', group: 'Claude' },
      ]),
    });
    ...
    const dropdown = parentEl.querySelector('.claudian-model-dropdown');
    expect(dropdown.children.map((child: any) => (
      child.hasClass('claudian-model-group')
        ? child.textContent
  ...
  it('should render group separators when models have group field', () => {
    ...
    // Provider groups keep source order; models reverse within each group.
