...
describe('PiSettingsTab', () => {
  ...
  it('renders provider controls in the active locale', () => {
    setLocale('zh-CN');
    render({ providerConfigs: { pi: {} } });

    expect(findSetting('连接').heading).toBe(true);
    expect(findSetting('CLI 路径').desc).toContain('Pi');
    expect(findSetting('模型').heading).toBe(true);
    expect(findSetting('权限与隔离').heading).toBe(true);
    expect(findSetting('权限处理').desc).toContain('只读工具和全部工具');
  ...
  it('does not duplicate provider enablement inside the detail panel', () => {
    ...
    expect(createdSettings.some(setting => setting.name === 'Enable Pi')).toBe(false);
  });
