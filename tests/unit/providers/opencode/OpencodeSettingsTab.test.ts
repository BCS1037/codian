...
describe('OpencodeSettingsTab', () => {
  ...
  it('renders provider-owned controls in the active locale', () => {
    setLocale('zh-CN');
    ...
    expect(findSetting('权限与隔离').heading).toBe(true);
    expect(findSetting('权限处理').desc).toContain('映射到 OpenCode 模式');
    expect(findSetting('命令与技能').heading).toBe(true);
    expect(findSetting('子代理').heading).toBe(true);
    expect(context.renderHiddenProviderCommandSetting).toHaveBeenCalledWith(
      expect.anything(),
      'opencode',
      expect.objectContaining({ name: '隐藏的命令与技能' }),
    );
  ...
  it('does not duplicate provider enablement inside the detail panel', () => {
    ...
    expect(createdSettings.some(setting => setting.name === 'Enable OpenCode')).toBe(false);
  });
  ...
  it('does not render in-app environment variable controls', () => {
    const plugin = createPlugin();
