...
describe('CodexModelPicker', () => {
  ...
  it('omits redundant model intro copy and visible-count summary', () => {
    setLocale('zh-CN');
    const plugin = createPlugin();
    plugin.settings.providerConfigs.codex.discoveredModels = [];
    plugin.settings.providerConfigs.codex.visibleModels = [];

    renderCodexModelPicker(createElement() as any, createContext(plugin), {} as any);

    expect(settingNames).not.toContain('可见模型');
    expect(settingDescriptions).toEqual([]);
    expect(elements.some(element => (
      element.classes.has('claudian-provider-model-picker-summary')
    ))).toBe(false);
    expect(elements.some(element => element.text.includes('No Codex models'))).toBe(false);
  });
  ...
  it('renders all app-server models selected by default and can clear the filter', async () => {
    ...
    expect(settingNames).not.toContain('Visible models');
