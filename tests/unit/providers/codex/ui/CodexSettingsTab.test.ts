...
describe('CodexSettingsTab', () => {
  ...
  it('does not duplicate provider enablement inside the detail panel', () => {
    ...
    expect(createdSettings.some(setting => setting.name === 'Enable Codex provider')).toBe(false);
  });

  it('renders models before access and isolation in the provider panel', () => {
    Object.defineProperty(process, 'platform', { value: 'darwin' });
    codexSettingsTabRenderer.render(createContainer(), createContext(createPlugin()), 'provider');
