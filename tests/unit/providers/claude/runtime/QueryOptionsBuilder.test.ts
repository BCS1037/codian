...
describe('QueryOptionsBuilder', () => {
  describe('needsRestart', () => {
    ...
    it('returns true when selected service environment changes', () => {
      const current = createMockPersistentQueryConfig({ environmentKey: 'ANTHROPIC_BASE_URL=a' });
      const next = createMockPersistentQueryConfig({ environmentKey: 'ANTHROPIC_BASE_URL=b' });

      expect(QueryOptionsBuilder.needsRestart(current, next)).toBe(true);
    });
