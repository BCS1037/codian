...
jest.mock('@/utils/env', () => ({
  parseEnvironmentVariables: jest.fn((value: string) => Object.fromEntries(
    value
      .split('\n')
      .map(line => line.trim())
      .filter(Boolean)
...
function createMockPlugin(overrides?: Partial<ColdStartQueryConfig['plugin']>) {
  return {
    ...
    getActiveEnvironmentVariables: jest.fn().mockReturnValue('ANTHROPIC_API_KEY=test-key'),
    ...overrides,
...
describe('runColdStartQuery', () => {
  ...
  describe('infrastructure errors', () => {
    it('rejects Claude launch without explicit developer authentication', async () => {
      ...
      const previous = Object.fromEntries(authKeys.map(key => [key, process.env[key]]));
      ...
      try {
        ...
        ).rejects.toThrow('Claude requires explicit API-key or supported cloud authentication.');
      } finally {
