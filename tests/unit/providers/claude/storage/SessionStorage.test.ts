...
describe('SessionStorage', () => {
  ...
  describe('loadMetadata', () => {
    ...
    it('loads legacy metadata and migrates it to .codian', async () => {
      const metadata = {
        id: 'session-legacy',
      ...
      expect(mockAdapter.write).toHaveBeenCalledWith(
        '.codian/sessions/session-legacy.meta.json',
        expect.any(String),
      );
  ...
  describe('toSessionMetadata - round trip', () => {
    it('round-trips the cached conversation preview without loading a transcript', async () => {
      const conversation: Conversation = {
        id: 'conv-preview-roundtrip',
        providerId: 'claude',
      ...
      expect(metadata.preview).toBe('First user message preview');

      const writtenContent = mockAdapter.write.mock.calls[0][1];
      mockAdapter.listFiles.mockResolvedValue([
        '.codian/sessions/conv-preview-roundtrip.meta.json',
      ]);
      mockAdapter.read.mockResolvedValue(writtenContent);

      const conversations = await storage.listAllConversations();

      expect(conversations[0].preview).toBe('First user message preview');
    });
