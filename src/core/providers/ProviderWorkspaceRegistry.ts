...
export class ProviderWorkspaceRegistry {
  ...
  static getVaultCommandRepository(providerId: ProviderId): ProviderVaultEntryRepository | null {
    return this.getServices(providerId)?.vaultCommandRepository ?? null;
  }
  ...
  static getMcpCatalog(providerId: ProviderId) {
    return this.getServices(providerId)?.mcpCatalog ?? null;
  }
  ...
  static supportsSettingsSection(
    providerId: ProviderId,
    section: ProviderSettingsSectionId,
  ): boolean {
    return this.getSettingsTabRenderer(providerId)?.sections?.includes(section) ?? false;
  }
