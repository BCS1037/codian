...
export class ModelSelector {
  ...
  renderOptions() {
    ...
    const currentModel = this.callbacks.getSettings().model;
    const models = this.getAvailableModels();
    const groupedModels: ProviderUIOption[][] = [];
    for (const model of models) {
      const currentGroup = groupedModels.at(-1);
      if (!currentGroup || currentGroup[0]?.group !== model.group) {
        groupedModels.push([model]);
      } else {
        currentGroup.push(model);
      }
    }
    const orderedModels = groupedModels.flatMap(group => [...group].reverse());
...
/** Keeps required controls visible when a narrow sidebar wraps the toolbar. */
export class InputToolbarLayoutController {
  ...
  private hasWrappedItems(): boolean {
    ...
    return firstRowCenter !== undefined
      && rowCenters.some(center => Math.abs(center - firstRowCenter) > ROW_CENTER_TOLERANCE);
  }
  ...
  private observeLayoutChanges(): void {
    ...
    if (typeof MutationObserverConstructor === 'function') {
      this.mutationObserver = new MutationObserverConstructor(() => this.scheduleLayout());
      this.mutationObserver.observe(this.toolbarEl, { childList: true, subtree: true });
    }
  }

  private scheduleLayout(): void {
    if (this.pendingLayout !== null) cancelScheduledAnimationFrame(this.pendingLayout);
