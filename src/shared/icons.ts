...
function createSvgElement(parent: Node, tagName: keyof SVGElementTagNameMap): SVGElement {
  return parent.createSvg(tagName);
}

export function appendMcpIcon(container: HTMLElement): void {
  container.empty();

  const svg = createSvgElement(container, 'svg');
  ...
  const title = createSvgElement(svg, 'title');
...
export function appendCheckIcon(container: HTMLElement): void {
  ...
  const polyline = createSvgElement(svg, 'polyline');
...
export function createProviderIconSvg(
  ...
  // Document.createSvg appends to the document root; build the icon in a detached fragment first.
  const fragment = createFragment();
  const svg = createSvgElement(fragment, 'svg');
...
function createProviderSvgChild(child: ProviderSvgChild, parent: SVGElement): SVGElement {
  const element = createSvgElement(parent, child.tag);
